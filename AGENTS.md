...
13. **big-ideate** — Chart an initiative too large for one session as a decision-ticket map on the issue tracker, then resolve one decision per session until discovery is complete and `create-prd` can run
